...
def parse_function_generator(with_label = True, use_superpix = False):
  ...
  def parse_function(serialized_example):
    ...
    def augmentation(image, label):
      ...
      shear_affine = tf.stack([1, -tf.math.sin(shear_angle), 0, 0, tf.math.cos(shear_angle), 0, 0, 0]);
      ...
      zoom_affine = tf.stack([scale, 0, (1 - scale) * image_with_label.shape[2] / 2, 0, scale, (1 - scale) * image_with_label.shape[1] / 2, 0, 0]);
      image = image_with_label[..., 0:1]; # image.shape = (1, 256, 256, 1)
      label = image_with_label[..., 1:2]; # label.shape = (1, 256, 256, 1)
      ...
      from_y = tf.clip_by_value(y + dy, 0, image_with_label.shape[1]); # new_y.shape = (height, width)
      from_x = tf.clip_by_value(x + dx, 0, image_with_label.shape[2]); # new_x.shape = (height, width)
      query_points = tf.reshape(tf.stack([from_y, from_x], axis = -1), (1, -1, 2)); # query_points.shape = (1, height * width, 2)
      interpolated = tfa.image.interpolate_bilinear(image_with_label, query_points); # interpolated.shape = (1, height * width, 2)
      image_with_label = tf.reshape(interpolated, (1, image_with_label.shape[1], image_with_label.shape[2], 2)); # image_with_label.shape = (1, height, width, 2)
      ...
      label = tf.cast(tf.equal(tf.expand_dims(label, axis = -1), tf.reshape(tf.range(3 if use_superpix else 6, dtype = tf.float32), (1, 1, -1))), dtype = tf.float32);
      return image, label;
